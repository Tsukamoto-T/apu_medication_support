...
void TransparentDetection::pocketCb(const sensor_msgs::PointCloud2ConstPtr &point_msg) {
  ...
  if(flag == 2){
    ...
    //時点の切り出し
    cv::Mat target_map_1,target_map_2,target_map_3,target_map_4;
    target_map_1 = cv::Mat(feature_map, cv::Rect(pre_rectangle[0],pre_rectangle[1]));
    target_map_2 = cv::Mat(feature_map, cv::Rect(pre_rectangle[2],pre_rectangle[3]));
    target_map_3 = cv::Mat(feature_map, cv::Rect(pre_rectangle[4],pre_rectangle[5]));
    target_map_4 = cv::Mat(feature_map, cv::Rect(pre_rectangle[6],pre_rectangle[7]));

    cv::Mat target_map_cv_1,target_map_cv_2,target_map_cv_3,target_map_cv_4;
    target_map_1.convertTo(target_map_cv_1,CV_8U,255);
    target_map_2.convertTo(target_map_cv_2,CV_8U,255);
    target_map_3.convertTo(target_map_cv_3,CV_8U,255);
    target_map_4.convertTo(target_map_cv_4,CV_8U,255);
    cv::imwrite("/home/stl/catkin_ws/src/apu_medication_support/picture/target_asa.png", target_map_cv_1);
    cv::imwrite("/home/stl/catkin_ws/src/apu_medication_support/picture/target_hiru.png", target_map_cv_2);
    cv::imwrite("/home/stl/catkin_ws/src/apu_medication_support/picture/target_yoru.png", target_map_cv_3);
    cv::imwrite("/home/stl/catkin_ws/src/apu_medication_support/picture/target_neru.png", target_map_cv_4);
    ...
    cv::MatND hist_1,hist_2,hist_3,hist_4;
    // 0 番目のチャンネルからヒストグラムを求めます．
    int channels[] = {0};
    int dims = 1; // 入力行列（画像）のチャンネル数

    calcHist( &target_map_cv_1, 1, channels, cv::Mat(), hist_1, dims, histSize, ranges,true,false );
    calcHist( &target_map_cv_2, 1, channels, cv::Mat(), hist_2, dims, histSize, ranges,true,false );
    calcHist( &target_map_cv_3, 1, channels, cv::Mat(), hist_3, dims, histSize, ranges,true,false );
    calcHist( &target_map_cv_4, 1, channels, cv::Mat(), hist_4, dims, histSize, ranges,true,false );

    //---ヒストグラム書き出し------
    std::ofstream ofs("/home/stl/catkin_ws/src/apu_medication_support/data/histgram_asa.csv");
    std::ofstream ofs2("/home/stl/catkin_ws/src/apu_medication_support/data/histgram_hiru.csv");
    std::ofstream ofs3("/home/stl/catkin_ws/src/apu_medication_support/data/histgram_yoru.csv");
    std::ofstream ofs4("/home/stl/catkin_ws/src/apu_medication_support/data/histgram_neru.csv");
    for( int binId = 0; binId < binNum; binId++ ){
      ofs << binId << "\t" << hist_1.at<float>(binId) << std::endl;
      ofs2 << binId << "\t" << hist_2.at<float>(binId) << std::endl;
      ofs3 << binId << "\t" << hist_3.at<float>(binId) << std::endl;
      ofs4 << binId << "\t" << hist_4.at<float>(binId) << std::endl;

    }

    //---target_map書き出し------
    std::ofstream ofs_2("/home/stl/catkin_ws/src/apu_medication_support/data/target_map_asa.csv");
    std::ofstream ofs_3("/home/stl/catkin_ws/src/apu_medication_support/data/target_map_hiru.csv");
    std::ofstream ofs_4("/home/stl/catkin_ws/src/apu_medication_support/data/target_map_yoru.csv");
    std::ofstream ofs_5("/home/stl/catkin_ws/src/apu_medication_support/data/target_map_neru.csv");

    //---target_mapデータpub------
    //std_msgs::Float32MultiArray feature_array;
    int f_num = 0;
    //feature_array.data.resize(target_map_1.cols*target_map_1.rows);
    for(int i = 0; i < target_map_1.cols; i++){
      for(int j = 0; j < target_map_1.rows; j++){
        //feature_array.data[f_num] = target_map.at<float>(j,i);
        ofs_2 << target_map_1.at<float>(j,i) << std::endl;
        ofs_3 << target_map_2.at<float>(j,i) << std::endl;
        ofs_4 << target_map_3.at<float>(j,i) << std::endl;
        ofs_5 << target_map_4.at<float>(j,i) << std::endl;
        //f_num++;
      }
    }
